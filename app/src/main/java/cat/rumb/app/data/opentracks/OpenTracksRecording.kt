...
object OpenTracksRecording {
    ...
    fun start(context: Context): Boolean {
        val api = discover(context) ?: run {
            cat.rumb.app.data.debug.DebugLog.w("Record", "start · cap API pública trobada")
            notFound(context); return false
        }
        cat.rumb.app.data.debug.DebugLog.i(
        ...
        return launch(context, api.pkg, api.startClass) {
            ...
            putExtra(EXTRA_STATS_CLASS, "cat.rumb.app.viewer.MapViewerActivity")
        }
    }

    fun stop(context: Context): Boolean {
        val api = discover(context) ?: run {
            cat.rumb.app.data.debug.DebugLog.w("Record", "stop · cap API pública trobada")
            notFound(context); return false
        }
        cat.rumb.app.data.debug.DebugLog.i("Record", "stop · ${api.pkg}/${api.stopClass}")
    ...
    private fun launch(context: Context, pkg: String, cls: String, extras: Intent.() -> Unit): Boolean {
        ...
        return runCatching { context.startActivity(build(context !is android.app.Activity)); true }
            ...
            .getOrElse {
                cat.rumb.app.data.debug.DebugLog.e("Record", "launch $pkg/$cls fallit", it)
                Toast.makeText(context, context.getString(cat.rumb.app.R.string.ot_launch_failed), Toast.LENGTH_LONG).show()
                false
            }
    }

    private fun notFound(context: Context) {
        Toast.makeText(context, context.getString(cat.rumb.app.R.string.ot_not_found), Toast.LENGTH_LONG).show()
    }
